...
export const EventForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      ...
      try {
        ...
        console.error(error);
        toast.error('Tải sự kiện để chỉnh sửa không thành công.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      toast.error('Lưu sự kiện không thành công.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-4xl mx-auto bg-white p-6 rounded-lg shadow space-y-5">
